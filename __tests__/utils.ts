...
describe(`utils`, () => {
  ...
  describe(`paginatedPath()`, () => {
    it(`Returns only one slash for the index page`, () => {
      expect(paginatedPath(`/`, 0, 3, false)).toEqual(`/`);
    });

    it(`Returns /2 for the second index page`, () => {
      expect(paginatedPath(`/`, 1, 3, false)).toEqual(`/2`);
    });

    it(`Passes pageNumber and numberOfPages to pathPrefix()`, () => {
      const pathPrefix = jest.fn(() => `/foo`);
      expect(paginatedPath(pathPrefix, 0, 3, false)).toEqual(`/foo`);
    ...
    it(`Returns empty string for the last + 1 page`, () => {
      expect(paginatedPath(`/foo`, 2, 2, false)).toEqual(``);
    });

    it(`Returns empty string for page -1`, () => {
      expect(paginatedPath(`/foo`, -1, 2, false)).toEqual(``);
    });

    it(`Appends trailing slash to the first page`, () => {
      expect(paginatedPath(`/foo`, 0, 1, true)).toEqual(`/foo/`);
    });

    it(`Appends trailing slash to remaining pages`, () => {
      expect(paginatedPath(`/foo`, 1, 2, true)).toEqual(`/foo/2/`);
    });

    it(`Does not append trailing slash to the index page`, () => {
      expect(paginatedPath(`/`, 0, 1, true)).toEqual(`/`);
    });
  });
